...
-- DISABLED: InspectionResponses table does not exist
...
BEGIN
    ...
    SELECT i.InspectionId,
           ...
           i.InspectorUserId,
           i.InspectionDate,
           i.InspectionDate, i.CASE WHEN Passed = 1 THEN 'Completed' ELSE 'Failed' END AS InspectionStatus,
           i.CASE WHEN Passed = 1 THEN 'Pass' ELSE 'Fail' END AS OverallResult,
           i.GpsLatitude,
           i.GpsLongitude,
           i.GpsAccuracyMeters,
           i.Notes,
...
BEGIN
    ...
    SELECT e.ExtinguisherId,
           ...
           l.LocationName,
           l.LocationCode,
           l.GpsLatitude AS LocationLatitude,
           l.GpsLongitude AS LocationLongitude,
           et.TypeName,
           et.TypeCode
    FROM [tenant_634F2B52-D32A-46DD-A045-D158E793ADCB].Extinguishers e
    INNER JOIN [tenant_634F2B52-D32A-46DD-A045-D158E793ADCB].Locations l
...
BEGIN
    ...
    WHERE TenantId=@TenantId
          ...
          AND InspectionDate BETWEEN @StartDate AND @EndDate;
    ...
    WHERE TenantId=@TenantId
          AND InspectionStatus='Completed'
          AND CASE WHEN Passed = 1 THEN 'Pass' ELSE 'Fail' END AS OverallResult='Fail'
          AND InspectionDate BETWEEN @StartDate AND @EndDate;
...
BEGIN
    ...
    SELECT i.InspectionId,
           ...
           i.CASE WHEN Passed = 1 THEN 'Pass' ELSE 'Fail' END AS OverallResult,
           i.Notes,
           i.it.TypeName AS InspectionTypeName,
    ...
    ORDER BY i.InspectionDate DESC OFFSET(@PageNumber - 1) * @PageSize ROWS FETCH NEXT @PageSize ROWS ONLY;
END;
GO
...
BEGIN
    ...
    UPDATE [tenant_634F2B52-D32A-46DD-A045-D158E793ADCB].Inspections SET Status = ISNULL(@Status, Status),
                                                                         GpsLatitude = ISNULL(@GpsLatitude, GpsLatitude),
                                                                         GpsLongitude = ISNULL(@GpsLongitude, GpsLongitude),
